...
> 部署流程本身不受影响。协作基建见根 `CONTRIBUTING.md`。
...
```bash
# 方式 A（推荐）：脚本生成填好强密钥的文件（openssl 随机 JWT/Cookie 密钥 + 超管口令，自动 chmod 600）
bash deploy/gen-secrets.sh /etc/haruhifanclub.env   # 生产
bash deploy/gen-secrets.sh                          # 本地 → 仓库根 .env（已存在则拒绝覆盖，--force 强制）

# 方式 B：手动从模板复制再填
cp deploy/env.sample /var/www/haruhifanclub-test/.env                                  # 测试站
cp deploy/env.sample /etc/haruhifanclub.env && chmod 600 /etc/haruhifanclub.env        # 生产
```

> **env 文件格式（systemd 坑，务必注意）**：`EnvironmentFile` 只把「行首 `#` / `;`」当注释，
> **不剥离行内注释**，也不解析 `export` 与 shell 引号。因此：值后**不要**跟 `# 注释`、**不要**写
> `export`、**不要**给值加引号——否则形如 `HARUHI_BIND=127.0.0.1:17777 # 监听` 的值会把注释算进去，
> SocketAddr 解析失败、服务起不来。`deploy/env.sample` 已按此规范（注释独立成行），照搬即安全；
> 手改 `/etc/haruhifanclub.env` 时也请遵守。dotenvy（本地 `.env`）会剥离行内注释、较宽松，但统一遵守最稳。
