...
版本约束：`package.json` 要求 Node ≥20、`packageManager: pnpm@10.11.0`；Rust 见 `rust-toolchain.toml`（stable）。
...
pnpm -r --filter "./apps/*" build      # 产物 → apps/<app>/dist（base 已指向各自子路径）
```
...
是 macOS/arm 的架构不匹配）。我们实际用的镜像是 `rust:1.87-bookworm`：
...
**release 必填，否则启动即失败（fail-fast，见 `backend/crates/core/src/config.rs`）：**

| 变量 | 说明 |
|---|---|
| `HARUHI_JWT_SECRET` | JWT 签名密钥，**≥32 位随机串**。缺失 → release 拒绝启动 |
...
| `HARUHI_CORS_ORIGINS` | 逗号分隔来源；同源部署可留空（默认仅 `PUBLIC_SITE_URL`） |
| `HARUHI_SUPERADMIN_USER` / `_PASSWORD` | 首启 seed 超管（仅 `core.db` 无用户时生效，见 `seed.rs`） |
| `PUBLIC_SITE_URL` | `https://haruyuki.cn`（CORS 默认来源 + 邮件链接） |
| `DASHSCOPE_API_KEY` | 空则 AI 审核离线放行 |
| `MAIL_*` / `RESEND_*` / `SMTP_*` | shop 下单邮件；`MAIL_ENABLED=false` 时不启 worker |
...
- `ExecStart=/var/www/haruhifanclub/bin/haruhi-server`，`EnvironmentFile=/etc/haruhifanclub.env`，`User=www-data`。
- `KillSignal=SIGTERM` + **`TimeoutStopSec=30`**：配合后端优雅关闭——收到 SIGTERM 后停收新请求、
  等在途请求结束、对每个库做 `wal_checkpoint(TRUNCATE)` 刷盘再退出（见 `main.rs`）。30s 内退完才不会被 SIGKILL，
  保证不丢数据/不损坏 WAL。
...
`EnvironmentFile=.env`，无 `User=`），逻辑同上。
...
- `upstream haruhi_backend { server 127.0.0.1:17777; }`，`location /api/` 反代并注入
  `X-Real-IP` / `X-Forwarded-For`（限流取 IP 依赖它）/ `X-Forwarded-Proto`，`proxy_read_timeout 120s`。
- `location /uploads/` 用 `alias` 直接服务磁盘文件（`expires 7d`，绕过后端提速）。
- 各前端 app 子路径 `alias` 到 `apps/<app>/dist` + SPA history 回退（`try_files ... /<base>/index.html`）：
  `/news/ /art/ /exam/ /library/(=novel) /shop/ /console/`；`/` 302 跳 `/news/`。
- `client_max_body_size 256m`（与后端 `DefaultBodyLimit` 对齐）。
...
同时把数据库里残留的旧 URL 前缀（`/shop-api/uploads/`、`/exam/api/uploads/`、`uploads/...` 等）重写到统一的
`/uploads/<module>/`。两种用法：
...
读取脚本顶部写死的旧项目路径（如 `NOVEL_SRC=/Volumes/data/haruhi-novel-reader` 等），
逐模块拷库 + `rsync` uploads + `sqlite3` 重写前缀。注意：
...
默认目标是 `-test` 部署根（`DEST_DATA=/var/www/haruhifanclub-test/data` 等）。
...
生产 `deploy/nginx.conf` 里 `ssl_certificate` 行默认注释，签好 `haruyuki.cn` 证书后取消注释。

## 9. 上线后自检

```bash
curl -s http://127.0.0.1:17777/api/health          # {"status":"ok",...}
