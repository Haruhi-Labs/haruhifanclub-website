...
已审计入口、壳层、主要视图和样式文件后，曾以 `shop` 与 `art` 作为第一批真实接入试点。当前反馈表明，试点中存在视觉退化，不能把“已接入”直接视为“设计验收通过”：

| App               | 真实 UI 主语                         | 当前问题                                                       | 第一阶段动作                                           |
| ----------------- | ------------------------------------ | -------------------------------------------------------------- | ------------------------------------------------------ |
| `@haruhi/shop`    | 商品卡、筛选、购物车、订单、后台表单 | 旧变量完整但散落 raw color；部分接入后视觉节奏和卡片质感退化   | 已接入但需要视觉复核，暂不作为成功样板                 |
| `@haruhi/art`     | 作品网格、筛选、上传、审核、弹窗     | 媒体比例和磨砂质感原本较成熟；部分接入后作品主位和界面层级变弱 | 已接入但需要视觉复核，暂不作为成功样板                 |
| `@haruhi/news`    | 导航、文章卡、搜索、发布后台         | 内容结构清楚且已有强烈报纸风格，直接套基础卡片会造成退化       | 第二批前先做视觉基线和保护清单，不直接替换 NavBar/Card |
...
2. **保护清单**：写清楚原页面哪些部分必须保留，例如 news 的报纸感、art 的作品主位、shop 的交易信息密度。
3. **接入模式选择**：决定本次是“无视觉变化的基线接入”，还是“完整流程重设计切片”，不能混成浅层套壳。
4. **退化即回退**：如果新版本只是“更统一”但更丑、更松散或信息更难扫读，必须回退并调整规范，而不是继续扩大接入。
...
4. 重设计完整对象：按新设计系统同时处理 Header、内容卡片/数据行、表单、状态、响应式和交互反馈。
5. 保留业务 recipe：商品卡、作品卡、书封卡、试卷卡先作为业务 recipe 完整设计，不直接抽成共享组件。
6. 提交证据：PR 附接入前后 390 / 768 / 1280px 截图、状态矩阵、a11y 检查、bridge 删除计划和回滚步骤。
...
| `@haruhi/shop` | 价格、库存、预售进度和订单路径真实                    | 旧蓝色大 header 和交易卡片需要整体重设计，浅接会削弱购买信任       | 暂不局部套皮；下一阶段应选商品列表 + 详情 + 购物车/结账作为完整交易切片。 |
