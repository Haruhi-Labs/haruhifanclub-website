...
| `download` | 凉宫春日资源站 | 夏日晴空蓝、明亮清爽、条理分明的资源索引                                | 晴空蓝 / 黄色信号        | 大面积深色、装饰盖过索引   |
| `voice`   | 春日语音工坊   | 活力橙声浪 + 信号黄的声音实验室；热闹有能量但不过载                      | 活力橙 / 黄色信号        | 橙色大面积铺底、装饰盖过播放器 |
...
| `download` | download | **深度**（DS 原生新站：tokens + components + `@haruhi/ui`）                                                                                  |
| `voice`   | voice   | **深度**（DS 原生新站：tokens + components + `@haruhi/ui`；TTS/RVC 转发站，服务状态横幅 + 播放器为站点自有样式）                              |
