...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/admin/site-config", get(misc::admin_get_site_config).put(misc::admin_put_site_config))
        // ---------- 优惠券 ----------
        ...
        .route("/admin/coupons/{id}/status", put(coupons::set_coupon_status))
        ...
        .route("/admin/contact-messages/{id}/status", put(misc::set_contact_message_status))
        // ---------- 商品 ----------
        ...
        .route("/products/{id}", put(products::update_product).delete(products::delete_product))
        ...
        .route("/orders/mark-spot-exported", put(admin_orders::mark_spot_exported))
        .route("/orders/mark-presale-exported", put(admin_orders::mark_presale_exported))
        .route("/orders/spot-export-data", get(admin_orders::spot_export_data))
        .route("/orders/import-tracking", post(admin_orders::import_tracking))
        .route("/orders/{id}/sub-orders/{subKey}/ship", put(admin_orders::ship_sub_order))
        .route("/admin/dashboard-summary", get(misc::dashboard_summary))
        // ---------- 订单（公开/通用）----------
        .route("/orders", get(admin_orders::list_orders).post(orders::create_order))
        .route("/orders/{id}", get(orders::get_order).delete(admin_orders::delete_order))
