<template>
  <div class="payment-container" v-if="order">
    <div class="text-center" style="margin-bottom: 2rem;">
        <h2 style="font-size: 1.5rem; font-weight: bold; color: var(--sos-text-primary); margin: 0 0 0.5rem 0;">订单已提交</h2>
        <p style="color: var(--sos-text-secondary); margin: 0;">请尽快完成支付，订单将保留 24 小时</p>
        <p style="color: var(--sos-text-secondary); margin: 0;">务必点击“我已完成支付”，避免订单失效。</p>
    </div>
    <div class="payment-grid">
        <div class="order-info-col">
            <label style="font-size: 0.75rem; color: var(--sos-text-secondary); text-transform: uppercase; margin-bottom: 0.25rem;">订单编号</label>
            <div class="flex-row items-center gap-2 order-sn-row" style="margin-bottom: 1.5rem;">
                <span class="order-sn">{{ order.id }}</span>
                <button @click="copy(order.id)" style="font-size: 0.75rem; background: var(--sos-accent-soft); color: var(--sos-accent); border: none; padding: 2px 6px; border-radius: 4px; cursor: pointer;">复制</button>
            </div>
            <label style="font-size: 0.75rem; color: var(--sos-text-secondary); text-transform: uppercase; margin-bottom: 0.25rem;">
                {{ isMergedOrder ? '本次追加应付' : '应付金额' }}
            </label>
            <div class="payment-amount">¥{{ displayPayableAmount }}</div>
            <div v-if="isMergedOrder" style="font-size: 0.75rem; color: var(--sos-text-tertiary); margin-bottom: 0.4rem;">
                旧订单金额 ¥{{ mergeSourceAmount }}，合并总金额 ¥{{ mergeMergedAmount }}
            </div>
            <div v-if="isMergedOrder && mergeShippingAdjustment < 0" style="font-size: 0.75rem; color: var(--sos-success); margin-bottom: 0.4rem;">
                已抵扣运费 -¥{{ mergeShippingRefund }}
            </div>
            <div v-else-if="isMergedOrder && mergeShippingAdjustment > 0" style="font-size: 0.75rem; color: var(--sos-danger); margin-bottom: 0.4rem;">
                追加运费 +¥{{ mergeShippingExtra }}
            </div>
            <div v-if="Number(order.discountAmount) > 0" style="font-size: 0.75rem; color: var(--sos-text-tertiary); margin-bottom: 0.5rem;">
                原价 ¥{{ order.originalTotal }}，优惠 -¥{{ order.discountAmount }}
            </div>
            <div style="font-size: 0.75rem; color: var(--sos-text-tertiary); margin-top: auto;">* 请务必支付准确金额</div>
        </div>
        <div class="qr-col">
            <!-- 支付方式切换 -->
            <div class="pay-method-switch">
                <button @click="payMethod = 'wechat'"
                    ...
                    :style="payMethod === 'wechat' ? 'background: #07c160; color: var(--sos-bg-surface); border: none; padding: 0.5rem 1.25rem; cursor: pointer; flex: 1; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 0.35rem; white-space: nowrap;' : 'background: var(--sos-bg-surface); color: var(--sos-text-secondary); border: none; padding: 0.5rem 1.25rem; cursor: pointer; flex: 1; font-size: 0.875rem; display: flex; align-items: center; justify-content: center; gap: 0.35rem; white-space: nowrap;'">
                    <i class="fab fa-weixin"></i> 微信
                </button>
                <button @click="payMethod = 'alipay'"
                    class="pay-method-btn"
                    :style="payMethod === 'alipay' ? 'background: #1677ff; color: var(--sos-bg-surface); border: none; padding: 0.5rem 1.25rem; cursor: pointer; flex: 1; font-size: 0.875rem; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 0.35rem; white-space: nowrap;' : 'background: var(--sos-bg-surface); color: var(--sos-text-secondary); border: none; padding: 0.5rem 1.25rem; cursor: pointer; flex: 1; font-size: 0.875rem; display: flex; align-items: center; justify-content: center; gap: 0.35rem; white-space: nowrap;'">
            ...
            <div class="qr-placeholder">
                ...
                <div v-else style="position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; flex-direction: column; gap: 0.25rem;">
                    <i class="fa fa-qrcode" style="font-size: 2.5rem; color: var(--sos-text-secondary); opacity: 0.7;"></i>
                    <span style="font-size: 0.75rem; color: var(--sos-text-tertiary);">后台未配置二维码</span>
                </div>
            </div>
            ...
            <div style="margin-top: 0.75rem; font-size: 0.75rem; color: var(--sos-text-secondary);">
                请备注: <span class="remark-code">{{ order.id.split('-').pop() }}</span>
            </div>
            <div style="margin-top: 0.5rem; font-size: 0.75rem; color: var(--sos-text-tertiary);">
                {{ PAYMENT_NOTE_TEXT }}
            </div>
        </div>
    </div>

    <p style="font-size: 0.75rem; color: var(--sos-text-tertiary); margin: 0 0 1.5rem 0; text-align: center;">管理员核对账单将有一定延迟，如果订单长期未确认，请联系我们。</p>
    ...
    <div v-if="helpModal" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 100;" @click.self="helpModal = false">
        <div class="help-modal-card">
            <h3 style="font-size: 1.125rem; font-weight: bold; color: var(--sos-text-primary); margin: 0 0 0.75rem 0;">添加好友转账</h3>
            ...
            <p style="font-size: 0.875rem; color: var(--sos-text-secondary); line-height: 1.6; margin: 0 0 0.5rem 0;">
                {{ FRIEND_TRANSFER_HELP_TEXT }}
            </p>
            <p style="font-size: 0.875rem; color: var(--sos-text-secondary); line-height: 1.6; margin: 0 0 1rem 0;">
                添加好友时请备注订单号，添加好友后即可点击“我已完成支付”，无需等待转账完成。
            </p>
            <div style="display: inline-block; background: var(--sos-success-soft); border: 1px solid var(--sos-success-soft); padding: 0.35rem 0.75rem; border-radius: 6px; margin-bottom: 1.25rem;">
                <span style="font-family: monospace; font-weight: bold; color: var(--sos-success); font-size: 0.9rem;">{{ order.id }}</span>
                <button @click="copy(order.id)" style="font-size: 0.7rem; background: var(--sos-success-soft); color: var(--sos-success); border: none; padding: 2px 6px; border-radius: 4px; cursor: pointer; margin-left: 0.5rem;">复制</button>
            </div>
            <div>
                <button @click="helpModal = false" style="background: var(--sos-bg-muted); color: var(--sos-text-secondary); border: none; padding: 0.5rem 2rem; border-radius: 8px; cursor: pointer; font-size: 0.875rem;">我知道了</button>
            </div>
        </div>
    </div>
