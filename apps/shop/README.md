...
- **商品与购物车** — 多图商品陈列与详情，购物车浮窗快速查看与改量
- **下单支付** — 填写收货地址、使用优惠券，微信 / 支付宝扫码后手动确认完成支付
- **订单查询** — 凭订单号查询订单状态与物流信息
- **预售** — 目标制（凑单）与固定日期两种模式，现货 + 预售混合订单自动拆分子订单、分组发货
- **优惠券** — 金额减免 / 百分比折扣，后台批次生成，结算页实时校验可用性与抵扣
- **邮件通知** — 订单状态变更自动通知（SMTP / Resend 双驱动，后端队列异步发送）
- **管理后台** — 商品、订单（批量导出 / CSV 快递单导入）、优惠券、留言、站点配置管理
- **数据统计** — 销售趋势、商品销售排行、转化率分析（Chart.js）
