...
需要临时使用正式站公开作品测试登录、关注、收藏等本地交互时，从仓库根运行：

```bash
pnpm dev:art:interactive
```

该命令把正式站公开作品元数据同步至本地 SQLite，在 `5203` 提供画廊；图片从正式站只读加载，账号和互动只写入本地数据库，不会污染正式站。常规运行和正式构建仍以自身后端为唯一数据源。
