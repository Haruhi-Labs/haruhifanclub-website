...
- **作品画廊** — 瀑布流 / 卡片展示，按标签、作者、内容类型（春日 / 其他）、来源（原创 / 搬运）筛选，支持按时间 / 热度 / 随机排序
- **作品投稿** — 多图上传，浏览器端压缩转 WebP 并保留原图；提交后经后端 AI 内容审核
- **匿名互动** — 无需登录，签名 Cookie 维持匿名身份，点赞（每日限额）与评论
- **积分体系** — 投稿得分，积分榜与按创作者 UID 查询积分明细（约定 10 绘画部积分 = 1 应援团积分）
- **特别授权查询** — 查询应援团对作品 / 创作者的授权信息
- **管理后台** — 作品审核 / 编辑、创作者（头像 / QQ）管理、评论管理（统一 JWT 登录 + RBAC）
- **移动端适配** — 响应式设计，兼顾桌面端与手机端
