...
3. **提交**——中间提交随手写即可（会被 squash 丢弃、不校验）。重点是后面开 PR 时**把标题按 [提交规范](#提交规范conventional-commits) 写好**。
...
> 因为采用 squash 合并，**PR 标题即最终落到 `main` 的提交信息**——请认真写 PR 标题（CI 会校验它）。
> PR 里的**中间提交会被 squash 丢弃**，怎么写都行、**不做校验**——你尽可以随手提交。
...
> **强制范围**：因用 squash 合并，CI 只对 **PR 标题**做校验（type 合法 + subject 非空），它就是最终落到 `main` 的提交信息。
> 下面的规范同样适用于 PR 标题；**PR 里的中间提交不校验**（会被 squash 丢弃），但也建议照此写，便于评审。
>
...
| 关卡 | 作用 |
| --- | --- |
| **pr-checks** | PR 标题语义检查（`amannn/action-semantic-pull-request`）：校验 **type** 合法、subject 非空（scope 不限、WIP 跳过、放行中文）。squash 下 PR 标题即最终提交信息，**只校验它**；中间提交会被 squash 丢弃、不校验 |
