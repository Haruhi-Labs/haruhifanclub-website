...
| Rust | **stable**（见 `rust-toolchain.toml`，含 rustfmt + clippy） | 后端编译 / lint / 测试 |
...
# rustup show 会自动按 rust-toolchain.toml 装好 stable + rustfmt + clippy
...
全仓统一使用 **Conventional Commits**，所有文件、所有人用**同一套**类型与作用域集合。
...
api-client
# 后端 crate
server  core  db  auth  media  ai  mail
# 跨领域
deploy  ci  docs  deps  release  repo
```
...
feat(library): 书签             # scope 必须是 novel（部署子路径是 /library/，但 scope 用 app 名）
fix(reader): 修复阅读器          # reader 不在 scope 集合里
```

> 小贴士：`scope` 用的是**作用域名**（如 `novel`），不是部署子路径（`/library/`）。
...
| 关卡 | 作用 |
| --- | --- |
| **pr-checks** | ① PR 标题语义检查（`amannn/action-semantic-pull-request`，校验 type/scope）；② 逐提交 commitlint（`wagoid/commitlint-github-action`，对中文友好：关闭 subject-case / subject-full-stop，header 放宽到 100） |
| **ci**（路径过滤） | 用 `dorny/paths-filter` 判断改了前端还是后端，分别跑：前端 `pnpm install --frozen-lockfile` + 各 app build（exam/console 含 `vue-tsc`）；后端 `cargo fmt --check` + `cargo clippy -D warnings` + `cargo test` + `cargo build --release` |
| **audit** | 依赖漏洞审计（`pnpm audit` + `cargo audit`），**信息性**、不阻断构建，但持续暴露 CVE |
